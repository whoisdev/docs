...
| --- | --- | --- | --- | --- |
| `nodeId` | `string` (UUID) | Yes | — | The notebook node to use as context. The model reads your connected documents. |
...
The Notebooks API works with OpenAI SDKs out of the box. Point the client at the Notebooks base URL and pass Notebooks-specific fields via `extra_body` (Python) or the second options argument (Node.js).
